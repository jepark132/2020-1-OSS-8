...
•	디코더 LSTM은 대상 시퀀스를 동일한 시퀀스로 변환하지만 향후 하나의 타임스테프로 오프셋되도록 교육되며, 이러한 맥락에서 "교사 강제력"이라고 불리는 교육 과정이다<br>
인코더의 상태 벡터를 초기 상태로 사용한다.<br>
효과적으로 디코더는 입력 순서에 따라 조건화된  목표물[t+1...], 주어진 표적[...t]을 생성하는 법을 배운다.<br>
