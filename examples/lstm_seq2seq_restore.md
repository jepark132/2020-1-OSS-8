...
•	우리는 도메인(영어문장)에서 인풋 순서형를 시작하고, 또 다른 도메인(프랑스어 문장)에서 corresponding target sequences를 시작한다.

•	인코더 LSTM은 입력 순서형를 2개의 상태 벡터로 변환한다(우리는 마지막 LSTM 상태를 유지하고 출력을 삭제함).

•	디코더 LSTM은 대상 순서형를 동일한 순서형로 변환하지만 향후 하나의 타임스테프로 오프셋되도록 교육되며, 이러한 맥락에서 "교사 강제력"이라고 불리는 교육 과정이다. 인코더의 상태 벡터를 초기 상태로 사용한다. 효과적으로 디코더는 입력 순서에 따라 조건화된  목표물[t+1...], 주어진 표적[...t]을 생성하는 법을 배운다.<br>

•	추론 모드에서 알 수 없는 입력 순서형를 디코딩하고자 할 때 우리는:<br>
   
     o	입력 순서형를 상태 벡터로 인코딩<br>
     o	크기 1의 타겟 순서형에서 시작(순서형의 시작 문자만)<br>
     o	디코더에 상태 벡터 및 1-char 목표 순서형를 입력하여 다음 문자에 대한 예측 생성<br>
     o	이러한 예측을 사용하여 다음 문자를 샘플링한다(우리는 단순히 argmax를 사용한다).<br>
     o	샘플 문자를 대상 순서형에 추가<br>
     o	순서형 종료 문자를 생성하거나 문자 한계에 도달할 때까지 반복<br>
